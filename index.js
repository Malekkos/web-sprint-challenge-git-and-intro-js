...
function getArtistByIndex(arr, numIndex) {
  return "the artist at index " + numIndex + " is " + arr[numIndex].name;
}
...
function listOfNames(arr) {
  ...
  // names = arr.push(`${arr[i].name}`);
  //}
  for (let i = 0; i < arr.length; i++) {
    //if (Object.keys(arr[i]) === "name") {
     // return names = Object.values(arr.name)
    //}
     names[i] = arr[i].name;
  }
  return names;
...
function removeArtist(arr, desIndexNum) {
  const newArtistsList = [...arr];
  for (let i = 0; i < arr.length; i++) {
    newArtistsList.splice(desIndexNum, 1);
